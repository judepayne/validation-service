"""
Loan entity helper class v2 - Domain-Driven Data Access Layer.

Standalone versioned helper for loan schema v2.0.0.
Maps domain concepts to v2 physical field names.

LOGICAL → PHYSICAL MAPPING (v2.0.0 model):
- id → id
- reference → reference_number  (was loan_number in v1)
- facility → facility_ref  (was facility_id in v1)
- category → loan_category  (new in v2)
- client → client_id
- collateral → collateral_id
- principal → financial.principal_amount
- balance → financial.outstanding_balance
- currency → financial.currency
- rate → financial.rate  (was financial.interest_rate in v1)
...
class LoanV2:
    """Helper class providing stable, domain-driven interface to loan v2.0.0 data."""
    ...
    def reference(self) -> str:
        self._record_access("reference", "reference_number")
        return self._data.get("reference_number", "")

    @property
    def facility(self) -> str:
        self._record_access("facility", "facility_ref")
        return self._data.get("facility_ref", "")

    @property
    def category(self) -> Optional[str]:
        self._record_access("category", "loan_category")
        return self._data.get("loan_category")
    ...
    def rate(self) -> Optional[float]:
        self._record_access("rate", "financial.rate")
        return self._data.get("financial", {}).get("rate")
